...
public class JSONHandler {
    ...
    public static <T> T jsonStringToObject(String jsonString, String pathToItem, String pathSplit, Class<T> objectType) {
        ObjectMapper objMapper = new ObjectMapper();

        try {
            JsonNode node = objMapper.readTree(jsonString);

            String[] pathNodes = pathToItem.split(pathSplit);
            for (String str : pathNodes) {
                node = node.path(str);
            }

            return objMapper.treeToValue(node, objectType);
        }
        catch (JsonProcessingException e) {
            e.printStackTrace();
            return null;
